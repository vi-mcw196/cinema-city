...
class CinemaHallSerializer(serializers.ModelSerializer):
    ...
    def validate_max_seats(self, value):
        if value < MIN_SEATS or value > MAX_SEATS:
            raise serializers.ValidationError('Max seats has to be between 1 and 100.')
        return value

    def validate_rows(self, value):
        if value < MIN_ROWS_COLUMNS or value > MAX_ROWS_COLUMNS:
            raise serializers.ValidationError('Rows has to be between 1 and 100.')
        return value

    def validate_columns(self, value):
        if value < MIN_ROWS_COLUMNS or value > MAX_ROWS_COLUMNS:
            raise serializers.ValidationError('Columns has to be between 1 and 100.')
        return value
